...
import {Mouse} from "@/lib/core/utils/mouse";
import {AABB, ForceQuadTree, QuadTreeNode} from "@/lib/core/utils/quadtree";
...
export class Scene {
    ...
    render() {
        ...
        //Quad tree debug
        if (this.force_quad_tree && this.is_debugging) {
            this.ctx.beginPath();
    ...
    private handleMultiTouchMove(e: TouchEvent) {
        if (!this.initial_touches || !this.initial_touches[1] || !this.initial_touches[0] || !e.touches[0] || !e.touches[1]) return;
    ...
    private handleMousePressed(e: MouseEvent | TouchEvent) {
        ...
        const val = e instanceof MouseEvent ? e : e.touches[0];
        if (!val)
            return;// Transform mouse coordinates to world space
        const pt = this.getTransformedPoint(val.clientX * devicePixelRatio, val.clientY * devicePixelRatio);
        this.prev_mouse_pos.set(pt.x, pt.y);
